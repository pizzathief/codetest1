...
# DHL specifies a conversion factor of 250 or 333 (as does Aust Post) , so conversion
# factor seems to be for all packages, not just air conditioners.
...
while not reached_end:
    ...
    dataframe_page = pandas.io.json.json_normalize(json.loads(data.text)['objects'])
...
# convert to a single dataframe.  Appending rows to a dataframe creates a new dataframe every time,
# and using the DataFrame.concat() function a lot is quite wasteful, so concat all at once

all_data = pandas.concat(pages)
all_data['cubic_weight'] = all_data['size.height'] * centimetre_to_metre_conversion *\
